...
from my_prompt import SYSTEM_FINAL_0129
from few_shot import examples_final
import json
...
if __name__ == '__main__':
    ...
    # query = "삼성전자와 애플의 주가를 비교해주고, 앞으로의 전망을 예측해줘."
    # query = "올해 전 세계에서 가장 많이 팔린 전기차 모델을 지역별로 분석해주고, 각 모델의 주요 특징과 성능을 비교해줘. 또한 전기차 시장의 성장 동향과 미래 전망도 알려줘."
    # query = "ChatGPT와 Gemini의 언어 모델 아키텍처는 어떻게 다른가? 각 모델이 학습 데이터를 다루는 방식에는 어떤 차이가 있는가?"
    query = "이번 주말에 제주도에서 자전거 여행을 하려고 하는데, 날씨와 추천할 만한 코스를 알려줘."
    # query = "한국어, 일본어, 중국어의 문법적 차이점을 비교하고, 이러한 차이가 기계 번역에 어떤 영향을 미치는지 설명해줘."
    print(get_sub_queries(query))